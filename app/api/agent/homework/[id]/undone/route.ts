...
/**
 * POST /api/agent/homework/:id/undone — 완료 취소 (실수 복구).
 */
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (item.doneAt === null) {
    return NextResponse.json({ ok: false, error: 'already not done' }, { status: 409 })
  }

  const res = await toggleItemDone(id, false)
  if (!res.ok) return NextResponse.json({ ok: false, error: res.error }, { status: 500 })

  return NextResponse.json({ ok: true, id, title: item.title })
}
