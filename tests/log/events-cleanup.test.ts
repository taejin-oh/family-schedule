...
describe('runEventsCleanup', () => {
  it('retention default is 30 days', () => {
    expect(EVENTS_RETENTION_DAYS).toBe(30)
  })

  it('deletes rows older than retention, keeps newer', () => {
    const appDb = makeDb()
    const now = new Date('2026-05-28T10:00:00').getTime()
    logEvent({ category: 'mutation', event: 'old',     ts: now - 31 * DAY }, { appDb })
    logEvent({ category: 'mutation', event: 'edge',    ts: now - 30 * DAY }, { appDb })
    ...
    expect(res.cutoff).toBe('2026-04-28')
  })
