    ...
    <div className="relative flex-shrink-0" ref={menuRef}>
      ...
      {open && (
        <>
          ...
          <div
            className={cn(
              'absolute right-0 z-20 mt-1 min-w-[11rem] rounded-md border bg-popover shadow-md',
          ...
          >
            ...
            {options.map((opt) => (
              <button
                ...
                className={cn(
                  'w-full text-left px-3 py-1.5 hover:bg-accent transition-colors flex items-center justify-between gap-3',
                  opt.date === currentDueDate && 'opacity-40 cursor-not-allowed'
                )}
              >
                <span>{opt.label}</span>
      ...
      {error && (
        <div className="absolute right-0 top-full z-30 mt-1 rounded bg-destructive px-2 py-1 text-xs text-destructive-foreground whitespace-nowrap shadow">
          {error}
        </div>
      )}
    </div>
