...
export function buildPrompt(input: {
  ...
  lines.push(
    ...
    `{"items":[{"title":"...","dueDate":"YYYY-MM-DD or null","notes":"...","confidence":0.95,"confidenceReason":"...","sourcePhotoIndex":0}, ...]}`,
    ``,
    `각 항목에 다음 필드를 추가하세요:`,
    `- confidence: 0~1 사이 추출 확신도. 문서에서 명확히 읽힌 경우 0.9 이상, 맥락으로 추론한 경우 0.5~0.8, 추측이 섞이거나 근거가 불충분하면 0.5 미만.`,
    `- confidenceReason: confidence가 0.7 미만이면 한국어 1줄(15~30자)로 짧은 이유를 적으세요. 예: "글씨가 흐려요", "단원 표기가 모호함", "날짜가 다른 행과 헷갈림". confidence가 0.7 이상이면 생략(필드 자체를 안 넣어도 됨).`,
